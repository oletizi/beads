...
public class Analyzer implements SegmentMaker {
  ...
  /* (non-Javadoc)
   * @see SegmentMaker#addSegmentListener(SegmentListener)
   */
  ...
  /* (non-Javadoc)
   * @see SegmentMaker#removeSegmentListener(SegmentListener)
   */
  ...
  private void setup(AudioContext ac, List<Class<? extends FeatureExtractor<?, ?>>> extractors, AnalysisSettings settings) {
    ...
    sd.addListener(d);
    sd.setDifferenceType(SpectralDifference.DifferenceType.POSITIVEMEANDIFFERENCE);
